...
import BudgetCategorySelect from '../../components/BudgetCategorySelect.vue';
        ...
        render: ({ row }) => {
            ...
            if (row.original.parent) {
                return h('div', { class: 'flex flex-col' }, [
                    h('span', { class: 'text-slate-900 text-sm' }, parentName),
                ]);
            }

            return h('span', { class: 'text-slate-400 italic' }, '-');
        },
    }),
...
// Separate state for parent's unit_ids (for default display only)
const defaultUnitIds = ref<number[]>([]);
...
const resetForm = () => {
    ...
    defaultUnitIds.value = [];
};

// Fetch parent category
const fetchParentData = async (parentId: number | null) => {
    if (!parentId) {
        if (!budgetCategoryForm.value.id) {
            budgetCategoryForm.value.budget_type = null;
            budgetCategoryForm.value.sub_budget_type = null;
            // Reset unit_ids to empty array when no parent selected
            budgetCategoryForm.value.unit_ids = [];
        }
        defaultUnitIds.value = [];
        return;
    }
    ...
    try {
        const response = await axios.get(
            `/api/v1/budget-categories/${parentId}`,
        );
        const parentData = response.data.data || response.data;

        if (parentData) {
            budgetCategoryForm.value.budget_type =
                parentData.budget_type || null;
            budgetCategoryForm.value.sub_budget_type =
                parentData.sub_budget_type || null;
            
            // Store parent's unit_ids as default
            defaultUnitIds.value = parentData.unit_ids || [];
            
            // Only set form unit_ids from parent if form hasn't been manually changed
            // For create mode, always set from parent as default
            if (!budgetCategoryForm.value.id) {
                budgetCategoryForm.value.unit_ids = parentData.unit_ids || [];
            }
            // For edit mode, keep existing unit_ids if already set
        }
    } catch (error) {
...
// Watch for parent_id changes
watch(
    () => budgetCategoryForm.value.parent_id,
    (newParentId) => {
        fetchParentData(newParentId);
    },
);
...
const isFormValid = computed(() => {
    const basicValid = budgetCategoryForm.value.name.trim() !== '';
...
const handleCreateBudgetCategory = async () => {
    loading.value = true;
    try {
        // Send unit_ids as is (could be parent's default or user-modified)
        await axios.post('/api/v1/budget-categories', budgetCategoryForm.value);
...
const handleUpdateBudgetCategory = async () => {
    loading.value = true;
    try {
        // Send unit_ids as is (user can modify from parent's default)
        await axios.put(
...
<template>
    <Head title="Sub Kategori Anggaran" />
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">
                        Sub Kategori Anggaran
                    </h1>
                    <p class="text-muted-foreground">
                        Pengaturan Sub Kategori Anggaran
                    </p>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <BudgetCategorySelect
                            id="create-parent"
                            v-model="budgetCategoryForm.parent_id"
                            placeholder="Pilih Kategori Induk"
                            :disabled="loading"
                            level="root"
                            :ignore-budget-type="true"
                        />
                    </div>
                    ...
                    <!-- Info Box untuk Tipe Anggaran -->
                    <div
                        v-if="budgetCategoryForm.parent_id"
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm text-blue-700">
                                    <p>
                                        Tipe Anggaran dan Sub Tipe Anggaran akan otomatis mengikuti Kategori Anggaran Induk yang dipilih.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <BudgetCategorySelect
                            ...
                            :disabled="loading"
                            level="root"
                            :exclude-id="budgetCategoryForm.id"
                            :ignore-budget-type="true"
                        />
                    </div>
        ...
        <FormDialog v-model:open="showDeleteDialog" size="sm">
            ...
            <FormDialog.Content>
                <div class="rounded-md border border-red-200 bg-red-50 p-4">
                    ...
                    <p
                        v-if="categoryToDelete?.parent"
                        class="mt-2 text-sm text-red-700"
                    >
                        Kategori Induk: {{ categoryToDelete.parent?.name }}
                    </p>
                </div>
            </FormDialog.Content>

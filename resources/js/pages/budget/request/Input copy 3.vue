...
const closeGoodsPanel = () => {
    if (currentActivityIndex.value >= 0 && currentItemIndex.value >= 0) {
        const item =
            budgetActivities.value[currentActivityIndex.value].request_items[
                currentItemIndex.value
            ];
        item.goods = [...currentGoods.value];

        const goodsTotal = currentGoods.value.reduce(
        ...
        if (goodsTotal > 0) {
            ...
            calculateActivityTotal(
                budgetActivities.value[currentActivityIndex.value].id,
            );
        }
    }
...
// Update item total from goods
const updateItemTotalFromGoods = () => {
    if (currentActivityIndex.value >= 0 && currentItemIndex.value >= 0) {
        const item =
            budgetActivities.value[currentActivityIndex.value].request_items[
                currentItemIndex.value
...
// Total goods in panel
const totalGoodsAmount = computed(() => {
    return currentGoods.value.reduce(
        (sum, good) => sum + (good.subtotal || 0),
        0,
    );
});

// ==================== TEMPLATE & DROPZONE FUNCTIONS ====================
...
// Download template Excel menggunakan library xlsx
const downloadTemplate = () => {
    try {
        // Buat workbook baru
        const wb = XLSX.utils.book_new();

        // Data untuk sheet "Data Barang"
        const headers = [
            'Nama Barang',
...
const parseCSVContent = (content: string) => {
    ...
    dataLines.forEach((line, index) => {
        const lineNumber = index + 2; // +2 karena index mulai 0 & baris 1 adalah header
        ...
        try {
            ...
            if (values.length < 6) {
                errorCount++;
                console.log(
                    `Baris ${lineNumber} ERROR: Kolom kurang (${values.length}/6)`,
                    line,
                );
                return;
            }
            ...
            if (!good) {
                errorCount++;
                console.log(
                    `Baris ${lineNumber} ERROR: Gagal membuat objek good`,
                    line,
                );
            } else {
                newGoods.push(good);
            ...
            errorCount++;
            console.log(`Baris ${lineNumber} ERROR:`, e, line);
        }
    });

    // Log ringkasan
    console.log(
        `Total error: ${errorCount} dari ${dataLines.length} baris data`,
    );
...
const parseExcelRows = (rows: any[][]) => {
    ...
    rows.forEach((row) => {
        try {
            // Konversi semua nilai ke string untuk konsistensi
...
// Create goods object from row data
const createGoodFromRow = (values: string[]): BudgetRequestItemGood | null => {
...
// Cleanup dropzone event listeners
onMounted(() => {
    // No need to add event listeners here as we use @drag events on the div
});
...
const handleUnitPrice = (event: Event, activityId: number, item: any) => {
    ...
    // Simpan nilai asli ke item.unit_price (langsung assign)
...
const buildFormData = () => {
    ...
    // 1. Kirim data meta form utama (tetap biasa)
    ...
    // 2. Bungkus data activities, items, goods
    const structuredActivities = budgetActivities.value.map(
        (activity, aIndex) => {
            // Mapping Request Items & Goods
            const items =
            ...
            // Mapping file dokumen (File biner asli di-append secara terpisah ke FormData)
            const documents =
                activity.files?.map((file, fIndex) => {
                    // Jika ini file baru (belum ada di server), kirim binary file-nya secara terpisah
                    if (!file.isExisting) {
                        fd.append(
    ...
    // 3. Masukkan seluruh array raksasa tadi sebagai SATU string JSON
    fd.append(
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="rounded-lg border bg-background p-6 shadow-sm">
            ...
            <form @submit.prevent="submitForm" class="space-y-6">
                <!-- ... (header form fields remain the same) ... -->
                                                                    ...
                                                                    >
                                                                        ...
                                                                        >
                                                                            ...
                                                                            >
                                                                                ...
                                                                                >
                                                                                    <div
                                                                                        class="w-64 space-y-1 rounded border bg-muted p-2 text-sm"
                                                                                    >
                                                                                        <div
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        >
                                                                                            ...
                                                                                            <Input
                                                                                                v-model="
                                                                                                    goodsForm.unit_price
                                                                                                "
                                                                                                type="text"
                                                                                                placeholder="0"
                                                                                                @input="
                                                                                                    handleGoodsUnitPrice
                                                                                                "
                                                                                                class="pl-10"
                                                                                            />
                                                                                        </div>

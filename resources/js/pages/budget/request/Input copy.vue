...
const loadBudgetRequest = async (id: number | string) => {
    ...
    try {
        ...
        alert(formData.value.budget_category_id);

        // Transformasi activities
        budgetActivities.value = transformActivities(
            data.request_activities || [],
        );

        // Set mode berdasarkan status
        const status = data.status?.toUpperCase();
        mode.value = status === 'DRAFT' ? 'edit' : 'view';

        // Update counters di background
...
onMounted(async () => {
    try {
        // Jika mode edit, load data utama
        if (props.id) {
...
const updateActivityItem = (
    ...
    if (selectedItem) {
        ...
        // Reset semua field terkait jika item dihapus
...
const buildFormData = () => {
    ...
    budgetActivities.value.forEach((activity, aIndex) => {
        ...
        activity.request_items?.forEach((item, iIndex) => {
            ...
            fd.append(
                `${itemPrefix}[activity_item_id]`,
                String(item.activity_item_id),
            );
            fd.append(`${itemPrefix}[description]`, item.description);
            fd.append(`${itemPrefix}[volume]`, String(item.quantity));
            fd.append(
                `${itemPrefix}[unit_measure_id]`,
                String(item.unit_measure_id),
            );
...
const validateForm = () => {
    ...
    // Validasi minimal satu kegiatan
    if (budgetActivities.value.length === 0) {
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="rounded-lg border bg-background p-6 shadow-sm">
            <!-- Loading Indicator - hanya tampil saat loading data dari API -->
            <div
                v-if="isLoading"
            ...
            <form @submit.prevent="submitForm" class="space-y-6">
                ...
                <div class="border-t pt-6">
                    ...
                    <!-- Loading state untuk tabel kegiatan -->
                    <div
                        v-if="isLoading"
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <p>
                                                Belum ada kegiatan. Klik
                                                "Tambah Kegiatan" untuk
                                                menambahkan.
                                            </p>
                                        </div>
                                    </TableCell>
                ...
                <div class="flex justify-end space-x-3 border-t pt-6">
                    <!-- Tombol Kembali untuk semua mode -->
                    <Button
                        type="button"
                        variant="outline"
                        @click="goBack"
                    ...
                    <!-- Actions untuk mode edit/create -->
                    <div v-if="!isReadOnly && !isLoading" class="flex space-x-2">
                        <!-- Tombol Reset selalu tampil jika bisa edit -->
                        <Button
                            type="button"
                            variant="outline"
                            @click="resetForm"
                        ...
                        <!-- Tombol Simpan Draft: Hanya tampil jika mode create atau status DRAFT -->
                        <Button
                            v-if="
                                mode === 'create' ||
                        ...
                        <!-- Tombol Submit (Ajukan) selalu tampil !isReadOnly -->
                        <Button
                            type="submit"
                            class="gap-2"
                            :disabled="isPosting"

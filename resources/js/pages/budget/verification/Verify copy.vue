...
const loadBudgetRequest = async (id: number | string) => {
    try {
        ...
        budgetActivities.value = transformActivities(
            data.budget_request_activities || [],
        );
...
const loadBudgetRequestApprovals = async (budgetRequestId: number | string) => {
    ...
    try {
        const response = await axios.get(`/api/v1/budget-requests/${budgetRequestId}/approvals`);
        const responseData = response.data as ApprovalsResponse;
        
        if (responseData.success && responseData.data && responseData.data.approvals) {
            ...
            verifiers.value = approvals.map((approval) => ({
                ...
                role_id: 0,
                department: '-',
                verificationStatus: mapApprovalStatusToVerificationStatus(approval.status),
            ...
            const completedApprovals = approvals.filter(
                approval => approval.status === 'approved' || approval.status === 'rejected' || approval.status === 'returned'
            );
            
            if (completedApprovals.length > 0) {
                verificationData.history = completedApprovals.map(approval => ({
                    step: approval.approval_level,
                    verifierId: approval.id,
                    verifierName: approval.role,
                    status: mapApprovalStatusToVerificationStatus(approval.status),
                    notes: approval.notes || '',
                    verifiedAt: approval.approved_at || new Date().toISOString(),
                }));
                
                // Set status awal dari history terakhir jika ada
                const lastHistory = verificationData.history[verificationData.history.length - 1];
                if (lastHistory) {
            ...
            if (headerStatus.value) {
                formData.value.status = mapHeaderStatusToFormStatus(headerStatus.value);
            }
...
const mapApprovalStatusToVerificationStatus = (status: string): Verifier['verificationStatus'] => {
...
const transformActivities = (activitiesData: any[]): BudgetActivity[] => {
    return activitiesData.map((activity: any) => {
        return {
            ...
            files: activity.documents?.map((att: any) => ({
                id: att.id,
...
const initializeVerificationData = () => {
    if (verificationData.history?.length === 0 && verifiers.value.length > 0) {
        ...
        verificationData.history = completedVerifiers.map(verifier => ({
            ...
            status: verifier.verificationStatus === 'completed' ? 'approved' : 'returned',
...
const isCurrentVerifier = computed(() => {
    ...
    if (currentVerifierData) {
        ...
        if (currentUserRoleId > 0 && currentVerifierData.role_id === currentUserRoleId) {
            return true;
        }
    }
...
const getCompletedStepsCount = computed(() => {
    return verifiers.value.filter(
        (v) => v.verificationStatus === 'completed' || v.verificationStatus === 'approved'
    ).length;
});
...
const isFormValid = computed(() => {
    return (
        ...
        verificationData.notes.trim().length > 0
    );
});
...
const submitVerification = async () => {
    ...
    try {
        ...
        const approvalData = {
            approval_level: currentStep,
            status: mapVerificationStatusToApprovalStatus(verificationData.status),
            notes: verificationData.notes,
        };

        const response = await axios.post(
            `/api/v1/budget-requests/${formData.value.id}/approvals/${currentStep}/process`,
            approvalData
        );
        ...
        if (verifierIndex !== -1) {
            verifiers.value[verifierIndex].verificationStatus = 
                ...
                verificationData.status === 'returned' ? 'rejected' : 'rejected';
            verifiers.value[verifierIndex].verifiedAt = new Date().toISOString();
            verifiers.value[verifierIndex].notes = verificationData.notes;
            verifiers.value[verifierIndex].decision = getDecisionFromStatus(verificationData.status);
        }
        ...
        if (response.data && response.data.data && response.data.data.current_level) {
            currentLevel.value = response.data.data.current_level;
        }

        if (currentStep < totalVerificationSteps.value) {
            const nextVerifierIndex = verifiers.value.findIndex(
                (v) => v.step === currentStep + 1,
            );
            if (nextVerifierIndex !== -1) {
                verifiers.value[nextVerifierIndex].verificationStatus = 'in_progress';
            ...
            const finalStatus = getFinalStatusFromOverall(verificationData.status);
            ...
            alert(messages[verificationData.status as keyof typeof messages] || 'Verifikasi selesai!');
        }

    } catch (error) {
        console.error('Error submitting verification:', error);
        alert('Terjadi kesalahan saat mengirim verifikasi.');
    } finally {
...
const saveAsDraft = async () => {
    ...
    try {
        const draftData = {
            ...
            currentVerifier: currentVerifier.value,
            currentVerifierStep: currentVerifierStep.value,
            isDraft: true,
        };
        ...
        alert('Draft verifikasi berhasil disimpan!');
    } catch (error) {
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div v-if="showLoadingSkeleton || isLoadingApprovals" class="rounded-lg border bg-white p-6 shadow-sm">
            <div class="animate-pulse">
                ...
                <div class="mb-6 border-b pb-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <div class="mt-2 h-4 w-96 bg-gray-200 rounded"></div>
                        </div>
                ...
                <div class="mb-8 rounded-lg border bg-gray-50 p-4">
                    <div class="mb-3 h-5 w-48 bg-gray-200 rounded"></div>
                ...
                <div class="mb-8 border-t pt-6">
                    <div class="mb-4 flex items-center justify-between">
                        <div>
                            ...
                            <div class="mt-2 h-4 w-64 bg-gray-200 rounded"></div>
                        </div>
                        <div class="h-4 w-32 bg-gray-200 rounded"></div>
                    </div>
                        ...
                        <div class="overflow-hidden rounded-lg border">
                            <div class="border-b bg-gray-50 px-4 py-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div>
                                        <div class="h-4 w-24 bg-gray-200 rounded"></div>
                                        <div class="mt-1 h-6 w-32 bg-gray-200 rounded"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="p-4">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-2 h-4 w-72 bg-gray-200 rounded"></div>
                                        </div>
                                    </div>
                                    <div>
                                        <div class="h-4 w-24 bg-gray-200 rounded"></div>
                ...
                <div class="border-t pt-6">
                    <div class="h-6 w-64 bg-gray-200 rounded mb-6"></div>
        ...
        <div v-else class="rounded-lg border bg-white p-6 shadow-sm">
            ...
            <div class="mb-6 border-b pb-4">
                <div class="flex items-center justify-between">
                    ...
                    <Badge :variant="getStatusVariant(formData.status)" class="px-3 py-1 text-sm">
                ...
                <div class="mt-4 grid grid-cols-1 gap-4 text-sm md:grid-cols-4">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-medium">{{ formData.request_no }}</span>
                    </div>

                    <div class="flex items-center gap-2">
                        <Calendar class="h-4 w-4 text-gray-400" />
                        <span class="text-gray-600">Tanggal Perencanaan:</span>
                        <span class="font-medium">{{ formatDate(formData.request_date) }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-medium">
                            {{ getCompletedStepsCount }} / {{ totalVerificationSteps }} Tahap
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div class="flex space-x-6">
                    ...
                    >
                        <div class="flex items-center gap-2">
                            ...
                            {{ tab.label }}
                            <Badge v-if="tab.badge" variant="secondary" class="ml-1 text-xs">
                                {{ tab.badge }}
                            </Badge>
                        </div>
                    </button>
            ...
            <div class="mt-6">
                ...
                <div v-if="activeTab === 'verifikasi'">
                    ...
                    <div class="mb-8 space-y-3 rounded-lg border bg-gray-50 p-4">
                        <div class="flex items-center justify-between">
                            <h3 class="font-medium text-gray-700">Informasi Perencanaan</h3>
                        </div>

                        <div class="grid grid-cols-2 gap-2 text-sm">
                            <span class="text-gray-500">Tahun Anggaran:</span>
                            <span class="font-medium">{{ formData.fiscal_year?.year || '-' }}</span>

                            <span class="text-gray-500">Periode Akademik:</span>
                            <span class="font-medium">{{ formData.academic_period?.display_name || '-' }}</span>

                            <span class="text-gray-500">Unit:</span>
                            <span class="font-medium">{{ formData.unit?.unit_name || '-' }}</span>

                            <span class="text-gray-500">Keterangan:</span>
                            <span class="font-medium whitespace-pre-wrap text-gray-700">{{ formData.description || '-' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="mb-8 border-t pt-6">
                        <div class="mb-4 flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-500">Rincian kegiatan dan item anggaran yang diajukan</p>
                            </div>
                            <div class="text-sm text-gray-600">
                                Total Kegiatan: <span class="font-medium">{{ budgetActivities.length }}</span>
                            </div>
                        </div>

                        <div class="space-y-4">
                            <div v-for="(activity, index) in budgetActivities" :key="activity.id" class="overflow-hidden rounded-lg border">
                                <div class="border-b bg-gray-50 px-4 py-3">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-3">
                                            <div class="flex h-8 w-8 items-center justify-center rounded-full bg-blue-100 font-bold text-blue-800">
                                                {{ index + 1 }}
                                            </div>
                                            <div>
                                                <h4 class="font-semibold text-gray-900">{{ activity.description }}</h4>
                                        ...
                                        <div class="text-right">
                                            ...
                                            <div class="text-lg font-bold text-blue-600">Rp {{ formatCurrency(activity.total_amount) }}</div>
                                        </div>
                                    </div>
                                </div>

                                <div class="p-4">
                                    <h5 class="mb-3 font-medium text-gray-700">Detail Item Anggaran:</h5>
                                        ...
                                        <Table>
                                            <TableHeader>
                                                <TableRow>
                                                    ...
                                                    <TableHead class="w-32">COA</TableHead>
                                                    <TableHead class="w-28">Volume</TableHead>
                                                    <TableHead class="w-28">Satuan</TableHead>
                                                    <TableHead class="w-40">Harga Satuan</TableHead>
                                            ...
                                            <TableBody>
                                                <TableRow v-for="(item, itemIndex) in activity.request_items" :key="item.id">
                                                    ...
                                                    <TableCell>
                                                        <div class="flex flex-col">
                                                            <span class="font-medium">{{ item.coa?.account_code }}</span>
                                                            <span class="text-xs text-gray-500">{{ item.coa?.account_name }}</span>
                                                        </div>
                                                    </TableCell>
                                                ...
                                                <TableRow class="bg-gray-50">
                                                    <TableCell colspan="6" class="text-right font-semibold">Subtotal Kegiatan:</TableCell>
                                                    <TableCell class="font-bold text-blue-600">Rp {{ formatCurrency(activity.total_amount) }}</TableCell>
                                                </TableRow>
                                            </TableBody>
                                        </Table>
                                    </div>
                    ...
                    <form v-if="isCurrentVerifier" @submit.prevent="submitVerification" class="border-t pt-6">
                        <div class="space-y-6">
                            <h3 class="text-lg font-semibold text-gray-900">
                                Verifikasi Pengajuan Anggaran - Tahap {{ currentVerifierStep }}
                            </h3>
                            ...
                            <div class="space-y-2">
                                ...
                                <Select v-model="verificationData.status" :disabled="!isCurrentVerifier">
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="returned">
                                            <div class="flex items-center gap-2">
                                                <FileEdit class="h-4 w-4 text-yellow-600" />
                                                <span>Revisi</span>
                                            </div>
                                        </SelectItem>
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center justify-between">
                                    ...
                                    <span class="text-xs text-gray-500">{{ verificationData.notes.length }}/800</span>
                                </div>
                                <Textarea
                        ...
                        <div class="flex justify-end space-x-3 border-t pt-6 mt-6">
                            <Button type="button" variant="outline" @click="cancelVerification" class="gap-2">
                                <X class="h-4 w-4" />
                                Batal
                            </Button>
                            <div class="flex space-x-2">
                                <Button
                                    ...
                                    variant="secondary"
                                    @click="saveAsDraft"
                                    :disabled="isSubmitting || !isFormValid || !isCurrentVerifier"
                                    class="gap-2"
                                >
                                    <Save class="h-4 w-4" />
                                    Simpan Draft
                                ...
                                >
                                    ...
                                    {{ isSubmitting ? 'Menyimpan...' : getSubmitButtonText() }}
                                </Button>
                            </div>
                        </div>
                    </form>
                    ...
                    <div v-else class="border-t pt-6">
                        <div class="rounded-lg bg-gray-50 p-6 text-center">
                            ...
                            <h3 class="mb-2 text-lg font-semibold text-gray-900">Verifikasi Telah Selesai</h3>
                            <p class="mb-4 text-sm text-gray-600">
                                Anda telah menyelesaikan verifikasi pada tahap ini. Silakan lihat riwayat verifikasi untuk detail lengkap.
                            </p>
                            <Button variant="outline" @click="activeTab = 'riwayat'">

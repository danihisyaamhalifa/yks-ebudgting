...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    ...
    {
        title: 'Disposisi Pencairan',
        href: '/disposisi-pencairan',
    },
    {
        title:
            mode.value === 'create'
                ? 'Pencatatan Disposisi Pencairan'
                : 'Detail Disposisi Pencairan',
        href: '',
    },
]);
...
const canPrintDispositionLetter = computed(() => {
    return !!selectedDisbursement.value && isDocumentRequired;
});
...
const generateDispositionLetterHTML = (): string => {
    ...
    //         <h1>Surat Disposisi Pencairan Dana</h1>
    //         <div class="number">No: ${data.disbursement_no || '-'}</div>
    //     </div>

    //     <div class="letter-body">
    //         <p>Menindaklanjuti pengajuan pencairan dana yang telah disetujui, bersama ini dimohon untuk segera melakukan pencairan dengan detail sebagai berikut:</p>

    //         <table class="info-table">
    //             <tr><td class="label">No. Pengajuan</td><td>: ${data.disbursement_no || '-'}</td></tr>
    //             <tr><td class="label">Tanggal</td><td>: ${formatDateIndonesian(data.disbursement_date || '')}</td></tr>
    //             <tr><td class="label">Unit</td><td>: ${data.request_header?.unit?.unit_name || '-'}</td></tr>
    //             <tr><td class="label">Kegiatan</td><td>: ${data.request_activity?.description || '-'}</td></tr>
    //             <tr><td class="label">Total Disetujui</td><td>: ${formatCurrencyWithRp(data.total_amount || 0)}</td></tr>
    //         </table>

    //         <p>Demikian surat disposisi ini dibuat untuk dapat dipergunakan sebagaimana mestinya.</p>
    //     </div>
    ...
    //         </div>
    //         <div class="signature-box">
    //             <div>${formatDateIndonesian(getTodayDate())}</div>
    //             <div class="name">...................</div>
    //             <div>Petugas</div>
    //         </div>
    //     </div>
...
const saveAsDraft = async () => {
    ...
    try {
        ...
        if (!formData.value.id) {
            router.visit('/disposisi-pencairan');
        }
    } catch (error: any) {
...
<template>
    <Head title="Disposisi Pencairan" />
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                Formulir pencatatan disposisi pencairan
                            </p>
                        </div>

...
const columns: ColumnDef<BudgetAccountability>[] = [
    createColumn({
        ...
        searchable: true,
        render: ({ row }) => {
            const disbursementHeader = row.original.fund_release.disbursement_header;
            const unitName =
    ...
    createActionColumn([
        {
            ...
            onClick: (row: BudgetAccountability) => viewAccountability(row),
        },
        {
            icon: EditIcon,
            variant: 'default',
            permissions: ['edit pertanggungjawaban_anggaran'],
            show: (row: BudgetAccountability) =>
                ['draft', 'returned'].includes(row.status),
            onClick: (row: BudgetAccountability) => editAccountability(row),
        },
        {
            icon: TrashIcon,
            variant: 'destructive',
            permissions: ['delete pertanggungjawaban_anggaran'],
            // show: (row: BudgetAccountability) =>
            //     ['submitted', 'draft', 'rejected'].includes(row.status),
            onClick: (row: BudgetAccountability) => deleteAccountability(row),
        },
    ]),
];
...
// Action handlers
const viewAccountability = (accountability: BudgetAccountability) => {
    router.visit(`/pertanggungjawaban/${accountability.id}/view`);
};

const editAccountability = (accountability: BudgetAccountability) => {
    router.visit(`/pertanggungjawaban/${accountability.id}/edit`);
};

const deleteAccountability = (accountability: BudgetAccountability) => {
    accountabilityToDelete.value = accountability;
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-muted-foreground">
                        Daftar pertanggungjawaban anggaran
                    </p>
                </div>

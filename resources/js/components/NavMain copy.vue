...
const onMenuHover = (event: MouseEvent, title: string, hasChildren: boolean) => {
    if (!isCollapsed()) return;
...
const updateDropdownPosition = () => {
    if (hoveredMenu.value && isCollapsed()) {
        ...
        const menuElement = document.querySelector(`[data-menu-title="${hoveredMenu.value}"]`);
        if (menuElement) {
    ...
    <SidebarGroup class="px-2 py-0">
                ...
                <div v-if="item.children?.length" class="relative">
                    <SidebarMenuButton
                        ...
                        :class="{
                            ...
                            'bg-muted': !isCollapsed() && (isMenuOpen(item.title) || isAnyChildActive(item)),
                    ...
                    >
                        ...
                        >
                            ...
                            <ChevronDown
                                ...
                                :class="{ 'rotate-180': isMenuOpen(item.title) }"
                            />
                        </div>
                    </SidebarMenuButton>
                ...
                <div v-else>
                    ...
                    >
                        <Link :href="item.href" class="flex items-center gap-2">
                            ...
                            <!-- Active indicator untuk collapsed mode -->
                            <div 
                                v-if="isCollapsed() && urlIsActive(item.href, page.url)"
                                class="absolute right-0 h-1.5 w-1.5 rounded-full bg-primary"
                                :style="{ right: '-4px' }"
                            />
                        </Link>
        ...
        <Teleport to="body">
            <div
                ...
                ref="dropdownRef"
                class="fixed z-50 min-w-[220px] max-w-[280px] rounded-lg border bg-popover shadow-lg animate-in fade-in-0 zoom-in-95 duration-200"
                :style="{
            ...
            >
                ...
                <div class="py-1">
                    ...
                    <template v-for="item in items" :key="item.title">
                        <div v-if="item.title === hoveredMenu">
                            ...
                            <div v-if="item.children?.length" class="px-2 py-1.5">
                ...
                <!-- Arrow indicator -->
                <div 
                    class="absolute h-2 w-2 rotate-45 border-l border-t bg-popover"
                    :style="{
                        left: '-5px',
                        top: '12px'
                    }"
